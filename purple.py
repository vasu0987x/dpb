...
temp_uploads = {}
user_numbers = {}  # Changed to support multiple numbers per user
...
def get_country(row):
    num = str(row[2])  # phone number column
...
def init_database():
    ...
    # User assignments table - Modified to support multiple numbers
    cursor.execute('''
...
def otp_processor_worker():
    ...
    while True:
        try:
            ...
            # Check for personal assignments - support multiple numbers
            assigned_users = []
            try:
                with get_db() as conn:
                    cursor = conn.cursor()
...
def get_service_code(sender: str) -> str:
    ...
    for service, code in SERVICE_CODES.items():
        if service.lower() in sender.lower():
            return code
    # If no match, return first 2 letters
...
def mask_number(number: str) -> str:
    ...
    if len(number) <= 4:
        return number
    # Show first 2 and last 4, mask middle
...
def login():
    ...
    res = session.post(
        "http://85.195.94.50/sms/signmein",
        data=payload,
        headers=HEADERS,
        timeout=15
    )
...
def send_random_numbers(chat_id, country=None, edit=False):
    ...
    # Save all 5 numbers for this user
...
def handle_copy_otp(call):
    """Handle OTP copy button - shows alert with OTP"""
    otp = call.data[5:]  # Remove "copy_" prefix
    try:
...
def handle_full_sms(call):
    """Handle full SMS view - shows popup with complete message only"""
    hash_id = call.data[8:]  # Remove "fullsms_" prefix
