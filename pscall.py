...
# ==================== PSCALL API CONFIG ====================
ZONE_API_BASE      = "http://pscall.net/restapi/smsreport"
ZONE_API_RECORDS   = 200                     # max records per request
...
def zone_build_url(fromdate=None, todate=None, searchnumber="", searchcli=""):
    params = {
        "key":    API_TOKEN,
        "start":  "0",
        "length": str(ZONE_API_RECORDS),
    }
    # pscall does not support date/number/cli filters in this endpoint,
    # but keep optional params for future compatibility
    if searchnumber:
...
def zone_fetch(fromdate=None, todate=None, searchnumber="", searchcli=""):
    """Fetch records from pscall.net SMS Report API with retry logic."""
    ...
    for attempt in range(1, 4):
        try:
            ...
            if result == "success":
                ...
                print(f"✅ pscallAPI: {len(records)} record(s)", flush=True)
                return records
            else:
                print(f"⚠️ pscallAPI: result={result} — {payload}", flush=True)
                return []
        except (requests.exceptions.ConnectionError,
                requests.exceptions.ChunkedEncodingError) as e:
            last_err = e
            print(f"⚠️ pscallAPI connection error (attempt {attempt}/3): {e}", flush=True)
            time.sleep(attempt * 3)
        except Exception as e:
            print(f"❌ pscallAPI unexpected error: {e}", flush=True)
            return []
    print(f"❌ pscallAPI failed after 3 attempts: {last_err}", flush=True)
    return []
...
def zone_parse_row(row: dict):
    """
    Convert a pscall.net API row into internal record format.
    API row: { "dateadded": "...", "num": "...", "cli": "...", "sms": "..." }
    Internal: { "dt": ..., "num": ..., "cli": ..., "message": ... }
    """
    try:
        number  = str(row.get("num",      "") or "").strip().lstrip("+").lstrip("0")
        sender  = str(row.get("cli",      "") or "").strip()
        message = zone_clean_message(str(row.get("sms", "") or ""))
...
def otp_scraper_thread():
    print("🟢 OTP Scraper Started (pscall.net API)", flush=True)
...
if __name__ == "__main__":
    print(f"🚀 OTP Bot v2.0 Starting — pscall.net API", flush=True)
